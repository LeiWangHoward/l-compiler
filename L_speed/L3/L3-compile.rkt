...
(define (compile-d L2_d var_x check_tail)
  (match L2_d
    ...
    [(list 'new-tuple args ...)
     (add-return-call
      `((eax <- (allocate ,(encode-const (length args)) 0))
    ...
    [(list 'make-closure args ...);;same as new-turple
     (add-return-call
      `((eax <- (allocate ,(encode-const (length args)) 0));3 -> 0
        ,@(make-mem-assigns 'eax args)
        (,var_x <- eax)) check_tail)]
    [(list 'closure-proc v)
     (add-return-call
    ...
    [(list (? symbol? f) args ...)
     (if (equal? check_tail #t)
         `(,@(assign-regs args);assign-regs also check if the register already has encoded value
           (tail-call ,f))
         `(,@(assign-regs args)
           (call ,f)
           (,var_x <- eax)))]
...
(define (compile-let var L3_d L3_e)
  (cond [(number? L3_d)
         `((,var <- ,(encode-const L3_d)) ,@(compile-e L3_e))]
        [(symbol? L3_d)
         `((,var <- ,L3_d) ,@(compile-e L3_e))]
        [else `(,@(compile-d L3_d var #f) ,@(compile-e L3_e))]))
